...
5. **`Frame` on `Face` explicitly; collapse the four-wedge seed.** Field: in place — every `Face` now carries a `frame: Matrix2D` (default identity), seeded into `computeComposites`/`computeImages` as the root's composite, and accounted for by `switchRoot`. Anchor-at-(0,0) convention not yet relaxed; once it is, the empty scene becomes a single all-ideal face.
