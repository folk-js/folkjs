...
// === CAMERA ===
const spin = -params.x * PI - PI / 4 + sinn(params.t) * 0.1
const elev = params.y * PI / 2 - PI / 4
const zoom = 0.9

// === BUILD THE SCENE ===

// Hexagon for extrusion demo (CW in math coords = CCW in screen coords where Y is down)
const hexagon = []
for (let i = 0; i < 6; i++) {
  const angle = -(i / 6) * PI * 2
  hexagon.push({ x: cos(angle) * 0.25, y: sin(angle) * 0.25 })
}

// Pentagon
const pentagon = []
for (let i = 0; i < 5; i++) {
  const angle = -(i / 5) * PI * 2 - PI / 2
  pentagon.push({ x: cos(angle) * 0.2, y: sin(angle) * 0.2 })
}

// Wobbly blob - irregular convex shape (like a dented ellipse)
const blob = []
for (let i = 0; i < 8; i++) {
  const angle = -(i / 8) * PI * 2
  // Vary the radius irregularly
  const r = 0.15 + 0.08 * sin(i * 1.7) + 0.05 * cos(i * 2.3)
  // Also offset the angle slightly
  const wobble = 0.15 * sin(i * 1.3)
  blob.push({ 
    x: cos(angle + wobble) * r * 1.3,  // stretch X
    y: sin(angle + wobble) * r * 0.9   // squish Y
  })
}

const segments = computeVisible([
  // "T" from two boxes
  box({ x: 0, y: -0.45, z: 0 }, { x: 1.6, y: 0.3, z: 0.3 }), 
  box({ x: 0, y: 0.25, z: 0 }, { x: 0.3, y: 1.1, z: 0.3 }),
  // Extruded hexagon, rotated
  rotate(extrude(hexagon, 0.4, { x: -1, y: 0.5, z: 0 }), { x: 0.3, y: 0.5, z: 0 }),
  // Extruded pentagon
  rotate(extrude(pentagon, 0.5, { x: 1, y: 0.3, z: 0 }), { x: -0.2, y: 0.3, z: 0.1 }),
  // Pyramid (not extruded - a built-in primitive)
  rotate(pyramid({ x: 1.1, y: -0.5, z: 0 }, { x: 0.4, y: 0.5, z: 0.4 }), { x: 0, y: 0.4, z: 0 }),
  // Extruded blob (asymmetric, wobbly convex polygon)
  rotate(extrude(blob, 0.2, { x: -1, y: -0.4, z: 0 }), { x: 0.5, y: -0.3, z: 0.2 }),
], spin, elev, zoom)
