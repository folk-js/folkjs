...
describe('Side', () => {
  ...
  it('ideal half-edge normalises its direction to unit length', () => {
    ...
    assert.ok(Math.abs(h.a.y - 0.8) < 1e-12);
  });
  ...
  it('side.kind discriminator covers all five geometric kinds', () => {
    ...
    for (const s of atlas.root.sides) {
      ...
      assert.equal(s.kind === 'arc', true);
      // @ts-expect-error — chord is not an arc
...
describe('Face', () => {
  ...
  it('accepts faces whose sides[0] is not at (0, 0) — anchor convention is relaxed', () => {
    ...
    assert.deepEqual(f.junctions(), [HomPoint.finite(1, 1), HomPoint.finite(10, 0), HomPoint.finite(0, 10)]);
  });
  ...
  it('accepts an all-ideal face (no finite vertex anywhere)', () => {
    ...
    assert.equal(f.sides[0].a.kind, 'ideal');
  ...
  it('successful construction wires next and face pointers on each half-edge', () => {
    ...
    assert.deepEqual(f.junctions(), [HomPoint.finite(0, 0), HomPoint.finite(10, 0), HomPoint.finite(0, 10)]);
  });
...
describe('isPolygonCCW', () => {
  ...
  it('handles all-ideal polygons (boundary lies on S¹)', () => {
    ...
    assert.equal(isPolygonCCW([idl(1, 0), idl(0, 1), idl(-1, 0), idl(0, -1)]), true);
    // Same directions in CW order — must register as not-CCW.
    assert.equal(isPolygonCCW([idl(1, 0), idl(0, -1), idl(-1, 0), idl(0, 1)]), false);
    // Three CCW directions covering all of S¹.
    assert.equal(isPolygonCCW([idl(1, 0), idl(-0.5, Math.sqrt(3) / 2), idl(-0.5, -Math.sqrt(3) / 2)]), true);
  });

  it('returns true for a CCW finite quadrilateral (k = 4)', () => {
    assert.equal(isPolygonCCW([fin(0, 0), fin(10, 0), fin(10, 10), fin(0, 10)]), true);
  });

  it('returns false for a CW finite quadrilateral (k = 4)', () => {
    assert.equal(isPolygonCCW([fin(0, 0), fin(0, 10), fin(10, 10), fin(10, 0)]), false);
  });
  ...
  it('returns false for a genuinely reflex quadrilateral', () => {
    ...
    assert.equal(isPolygonCCW([fin(0, 0), fin(10, 0), fin(5, -5), fin(0, 10)]), false);
  });
  ...
  it('handles a k = 4 face with two finite + two ideal vertices', () => {
    ...
    assert.equal(isPolygonCCW([fin(0, 0), fin(10, 0), idl(1, 1), idl(-1, 1)]), true);
  });
  ...
  it('accepts a k = 4 with a colinear chain vertex on a long side', () => {
    ...
    assert.equal(isPolygonCCW([fin(0, 0), fin(10, 0), idl(1, 0), idl(0, 1)]), true);
  });
  ...
  it('accepts a strip rectangle (k = 4 with same-ideal-direction ends)', () => {
    ...
    assert.equal(isPolygonCCW([fin(0, 0), fin(0, 5), idl(-1, 0), idl(-1, 0)]), true);
  });
...
describe('createInitialAtlas', () => {
  ...
  it('every face has anchor at finite (0, 0) and 2 ideal half-edges', () => {
    ...
    for (const f of atlas.faces) {
      assert.equal(f.sides[0].a.kind, 'finite');
      assert.equal(f.sides[0].a.x, 0);
      assert.equal(f.sides[0].a.y, 0);
      assert.equal(f.sides[1].a.kind, 'ideal');
      assert.equal(f.sides[2].a.kind, 'ideal');
    }
  });
  ...
  it('twins 8 half-edges along the cardinal half-axes; 4 at-infinity boundaries are untwined', () => {
    ...
    for (const he of atlas.sides) {
      if (he.twin) {
        ...
        assert.equal(he.kind === 'arc', false, 'twined half-edge should not be at-infinity');
      } else {
        boundary++;
        assert.equal(he.kind === 'arc', true, 'untwined half-edge should be at-infinity');
      }
    }
...
describe('aroundJunction', () => {
  it('walks all 4 half-edges around the central finite junction in the seed', () => {
    ...
    assert.equal(start.a.kind, 'finite');
    ...
    for (const he of fan) {
      assert.equal(he.a.kind, 'finite');
      assert.equal(he.a.x, 0);
      assert.equal(he.a.y, 0);
    }
  ...
  it('walks both half-edges around an ideal cardinal junction (boundary fan)', () => {
    ...
    const start = atlas.sides.find((h) => h.a.kind === 'ideal' && h.kind !== 'arc')!;
    const fan = [...aroundJunction(start)];
    ...
    for (const he of fan) {
      assert.equal(he.a.kind, 'ideal');
      // Same physical direction (in their respective face frames, which are
      // identity-related in the seed).
      assert.ok(Math.abs(he.a.x - start.a.x) < 1e-12);
      assert.ok(Math.abs(he.a.y - start.a.y) < 1e-12);
    }
  });
});
...
describe('validateAtlas', () => {
  it('accepts a face whose sides[0] is not at (0, 0) — anchor convention is relaxed', () => {
    ...
    // identity by construction. We rewrite the underlying Stitch in place
...
describe('Atlas.computeComposites', () => {
  ...
  it('reachable faces propagate non-identity transforms (synthetic)', () => {
    ...
    // Manually tamper with the Stitch's transforms (skipping the validation
    // shape `stitch()` enforces). After this the junction-correspondence
...
describe('Atlas.computeImages', () => {
  ...
  it('manual self-twinning quad produces multiple images of the same face', () => {
    ...
    stitch(atlas, he1, he3, M.fromTranslate(-w, 0));
  ...
  it('respects maxImagesPerFace on a self-twinning loop', () => {
    ...
    atlas.faces = [f];
    atlas.sides = [he0, he1, he2, he3];
    stitch(atlas, he1, he3, M.fromTranslate(-1, 0));
...
describe('Link', () => {
  ...
  it('link() throws when either endpoint is not in the atlas', () => {
    ...
    assert.throws(() => link(atlas1, atlas1.faces[0], atlas2.faces[0], M.fromValues()), /to face not in atlas/);
    assert.throws(() => link(atlas1, atlas2.faces[0], atlas1.faces[0], M.fromValues()), /from face not in atlas/);
  });
  ...
  it('reachability follows links: a face only connected via Link is still reachable', () => {
    ...
    const orphan = new Face([
      new Side(HomPoint.finite(100, 100)),
      new Side(HomPoint.finite(110, 100)),
      new Side(HomPoint.finite(105, 110)),
    ]);
  ...
  it('recursive zoom: a self-link of the root tiles via maxImagesPerFace', () => {
    ...
    const scales = rootImages.map((img) => img.composite.a).sort((x, y) => y - x);
  ...
  it('region image is path-independent after a line-cut adjacent to a wrapped region', () => {
    ...
    atlas.root = ne;
    const rBeforeAtNE = atlas
      .computeImages({ maxDepth: 8, maxImagesPerFace: 1 })
...
describe('rescaleFaceFrame', () => {
  ...
  it('multiplies finite boundary coords by R and conjugates twin transforms', () => {
    ...
    assert.equal(L.bottom.a.x, 0);
    assert.equal(L.right.a.x, 2);
    assert.equal(L.top.a.x, 2);
    assert.equal(L.top.a.y, 2);
    assert.equal(L.left.a.y, 2);
    // R.face's coords unchanged.
    assert.equal(R.right.a.x, 1);
    assert.equal(R.top.a.y, 1);
  ...
  it('round-trips: rescale by R then by 1/R restores all coordinates and transforms', () => {
    ...
    assert.ok(Math.abs(L.right.a.x - before.lRightOx) < 1e-9);
    assert.ok(Math.abs(L.top.a.y - before.lTopOy) < 1e-9);
  ...
  it('R = 1 is a no-op', () => {
    const { atlas, L } = makeTwoCellStrip();
    const beforeX = L.right.a.x;
    const beforeY = L.right.a.y;
    rescaleFaceFrame(atlas, L.face, 1);
    assert.equal(L.right.a.x, beforeX);
    assert.equal(L.right.a.y, beforeY);
  });
  ...
  it('rescaling a centroid-anchored sub-face leaves its perimeter projection invariant', () => {
    ...
    // is its inverse.
    stitch(atlas, Lq.right, Rq.left, M.fromTranslate(-2, 0));
...
describe('walkLine', () => {
  it('walks a line through 2 wedges of the seed atlas', () => {
    ...
    assert.ok(lastExit.he.kind === 'arc' || lastExit.he.twin === null);
  });
  ...
  it('chain ends on at-infinity arcs at both extremes', () => {
    ...
    assert.ok(first.entry?.he.kind === 'arc' || first.entry?.he.twin === null, 'chain start: entry at infinity');
    assert.ok(last.exit.he.kind === 'arc' || last.exit.he.twin === null, 'chain end: exit at infinity');
    // Exactly one host.
  ...
  it('throws when seam is not strictly inside host', () => {
    ...
    assert.throws(() => walkLine(host, { x: 0, y: 0 }, { x: 1, y: 0 }), /not strictly interior/);
  });

  it('throws on zero-length direction', () => {
    const atlas = createInitialAtlas();
    const host = atlas.locate({ x: 3, y: 5 })!;
    assert.throws(() => walkLine(host, { x: 3, y: 5 }, { x: 0, y: 0 }), /zero-length/);
  });
});
...
describe('createFace', () => {
  it('builds a triangle face from three junctions and registers it with the atlas', () => {
    ...
    const face = createFace(atlas, [HomPoint.finite(0, 0), HomPoint.finite(1, 0), HomPoint.finite(0, 1)]);
    ...
    assert.deepEqual(face.junctions(), [HomPoint.finite(0, 0), HomPoint.finite(1, 0), HomPoint.finite(0, 1)]);
  ...
  it('builds a digon face with anchors on both ideal-ideal chord sides', () => {
    const atlas = createAllIdealAtlas();
    const face = createFace(atlas, [HomPoint.idealDir(1, 0), HomPoint.idealDir(-1, 0)], {
  ...
  it('passes through an explicit frame', () => {
    ...
    const face = createFace(atlas, [HomPoint.finite(0, 0), HomPoint.finite(1, 0), HomPoint.finite(0, 1)], { frame });
    assert.ok(M.equals(face.frame, M.fromTranslate(7, -3)));
  });

  it('rejects fewer than 2 sides', () => {
    const atlas = createAllIdealAtlas();
    assert.throws(() => createFace(atlas, [HomPoint.finite(0, 0)]), /need at least 2 sides/);
  });
  ...
  it('rejects out-of-range anchor index', () => {
    ...
    assert.throws(
      () =>
        createFace(atlas, [HomPoint.idealDir(1, 0), HomPoint.idealDir(-1, 0)], {
...
describe('deleteFace', () => {
  ...
  it('removes every side of a deleted face from atlas.sides', () => {
    const atlas = createAllIdealAtlas();
    const face = createFace(atlas, [HomPoint.finite(0, 0), HomPoint.finite(2, 0), HomPoint.finite(1, 1)]);
  ...
  it('refuses to delete a face that is the source of a Link', () => {
    const atlas = createAllIdealAtlas();
    const a = createFace(atlas, [HomPoint.finite(0, 0), HomPoint.finite(1, 0), HomPoint.finite(0, 1)]);
    const b = createFace(atlas, [HomPoint.finite(5, 5), HomPoint.finite(6, 5), HomPoint.finite(5, 6)]);
  ...
  it('handles the unbind-then-delete pattern: a previously stitched face deletes after unstitch', () => {
    ...
    const atlas = createAllIdealAtlas();
    const a = createFace(atlas, [HomPoint.finite(0, 0), HomPoint.finite(2, 0), HomPoint.finite(1, 2)]);
    const b = createFace(atlas, [HomPoint.finite(2, 0), HomPoint.finite(0, 0), HomPoint.finite(1, -2)]);
...
describe('subdivideSide', () => {
  ...
  it('subdivides a finite-finite half-edge after a prior spoke subdivision', () => {
    ...
    const ff = atlas.sides.find((h) => h.a.kind === 'finite' && h.next.a.kind === 'finite' && h.twin !== null);
  ...
  it('the inserted vertex maps consistently across the twin (T·newVertex agrees in G)', () => {
    ...
    assert.ok(Math.abs(tw_B.a.x - expectedInG.x) < 1e-9, 'twin newVertex x mismatch');
  ...
  it('throws when point is at the start endpoint', () => {
    const atlas = createInitialAtlas();
    const spoke = findPlusXSpoke(atlas);
    assert.throws(() => subdivideSide(atlas, spoke, { x: 0, y: 0 }), /not strictly between endpoints/);
  });
  ...
  it('throws when point is not on the edge', () => {
    ...
    assert.throws(() => subdivideSide(atlas, spoke, { x: 5, y: 7 }), /not on the edge|not strictly between/);
  });
...
describe('joinSidesAtVertex', () => {
  ...
  it('round-trip on a finite-finite segment (subdivide a region edge)', () => {
    ...
    const atlas = createAllIdealAtlas();
    const a = createFace(atlas, [HomPoint.finite(0, 0), HomPoint.finite(4, 0), HomPoint.finite(2, 3)]);
    const b = createFace(atlas, [HomPoint.finite(4, 0), HomPoint.finite(0, 0), HomPoint.finite(2, -3)]);
  ...
  it('refuses to join non-collinear sides', () => {
    ...
    const atlas = createAllIdealAtlas();
    const tri = createFace(atlas, [HomPoint.finite(0, 0), HomPoint.finite(4, 0), HomPoint.finite(2, 3)]);
    ...
    assert.throws(() => joinSidesAtVertex(atlas, r.faceHalves[0]), /not collinear|not on segment/);
  });
  ...
  it('refuses asymmetric stitch pair (only one of the two is stitched)', () => {
    ...
    assert.throws(() => joinSidesAtVertex(atlas, r.faceHalves[0]), /side is stitched but side.next is not/);
  });

  it('refuses when face has fewer than 3 sides', () => {
    const atlas = createAllIdealAtlas();
    const digon = createFace(atlas, [HomPoint.idealDir(1, 0), HomPoint.idealDir(-1, 0)], {
      anchors: new Map<number, { x: number; y: number }>([
        [0, { x: 0, y: 0 }],
        [1, { x: 0, y: 1 }],
      ]),
    });
    assert.throws(() => joinSidesAtVertex(atlas, digon.sides[0]), /fewer than 3 sides/);
  });
  ...
  it('refuses when paired stitch transforms have diverged', () => {
    ...
    r.faceHalves[1].stitch!.transformAtoB = M.fromTranslate(0.1, 0);
    r.faceHalves[1].stitch!.transformBtoA = M.fromTranslate(-0.1, 0);
    assert.throws(() => joinSidesAtVertex(atlas, r.faceHalves[0]), /transforms diverged/);
  });
});
...
describe('splitFaceAtVertices', () => {
  ...
  it('splits a k=4 face into two triangles via a non-degenerate chord (v1 ↔ v3)', () => {
    ...
    assert.equal(result.face.sides.length, 3);
    assert.equal(result.fresh.sides.length, 3);
    assert.doesNotThrow(() => validateAtlas(atlas));
  });
  ...
  it('cuts a single all-ideal face along an ideal-ideal chord (with anchor)', () => {
    ...
    const idxMinusX = verts.findIndex((v) => v.kind === 'ideal' && Math.abs(v.x + 1) < 1e-9 && Math.abs(v.y) < 1e-9);
    ...
    for (const f of [result.face, result.fresh]) {
      ...
      const arcs = f.sides.filter((h) => h.kind === 'arc');
      const chords = f.sides.filter((h) => h.kind === 'chord');
    ...
    const c0 = result.face.sides.find((h) => h.kind === 'chord')!;
    const c1 = result.fresh.sides.find((h) => h.kind === 'chord')!;
  ...
  it('throws when an ideal-ideal chord is requested without a anchor', () => {
    ...
    const idxPlusX = verts.findIndex((v) => v.kind === 'ideal' && Math.abs(v.x - 1) < 1e-9 && Math.abs(v.y) < 1e-9);
    const idxMinusX = verts.findIndex((v) => v.kind === 'ideal' && Math.abs(v.x + 1) < 1e-9 && Math.abs(v.y) < 1e-9);
    assert.throws(() => splitFaceAtVertices(atlas, atlas.root, idxPlusX, idxMinusX), /requires a finite anchor/);
  });
  ...
  it('throws when ideal-ideal chord endpoints are not antipodal', () => {
    ...
    const dist = Math.min(Math.abs(idxPlusX - idxPlusY), verts.length - Math.abs(idxPlusX - idxPlusY));
    assert.ok(dist >= 2, 'expected non-adjacent vertices for the test setup');
    assert.throws(() => splitFaceAtVertices(atlas, root, idxPlusX, idxPlusY, { x: 0, y: 0 }), /must be antipodal/);
  });
  ...
  it('handles repeated chord splits without violating invariants', () => {
    ...
    const atlas = createInitialAtlas();
    const spokeX = atlas.sides.find((h) => h.a.kind === 'finite' && h.next.a.kind === 'ideal' && h.next.a.x === 1)!;
    const ne = spokeX.face;
    subdivideSide(atlas, spokeX, { x: 5, y: 0 });
    const spokeY = ne.sides.find((h) => h.a.kind === 'ideal' && h.a.x === 0 && h.a.y === 1)!;
  ...
  it('preserves wrap topology when splitting a wrapped face into halves', () => {
    ...
    const subA = split.face;
    const subB = split.fresh;
    const aWrap = subA.sides.find((he) => he.twin && he.twin.face === subB && he.twin !== split.freshChordSide);
    const bWrap = subB.sides.find((he) => he.twin && he.twin.face === subA && he.twin !== split.faceChordSide);
...
describe('splitFaceAlongChord', () => {
  ...
  it('cuts a face when both endpoints land at existing vertices (no subdivision)', () => {
    ...
    const v1Hit: BoundaryHit = {
      kind: 'finite',
      he: quad.sides[1],
      point: { x: 5, y: 0 },
      u: 0,
    };
    const v3Hit: BoundaryHit = {
      kind: 'ideal',
      he: quad.sides[3],
      idealDir: { x: 0, y: 1 },
    };
    ...
    assert.equal(atlas.faces.length, facesBefore + 1);
    assert.equal(result.face.sides.length + result.fresh.sides.length, 6);
    assert.doesNotThrow(() => validateAtlas(atlas));
  });
  ...
  it('cuts a face when one endpoint lands on an at-infinity arc (subdivides arc)', () => {
    ...
    const spoke = wedge.sides.find((h) => h.a.kind === 'finite' && h.next.a.kind === 'ideal' && h.next.a.x === 1)!;
  ...
  it('cuts an all-ideal face along a chord between two arc points (with anchor)', () => {
    ...
    const arcPlusX = root.sides.find(
      (h) => h.kind === 'arc' && Math.abs(h.a.x - 1) < 1e-9 && Math.abs(h.next.a.x) < 1e-9,
    )!;
  ...
  it('throws when both hits are on the same half-edge', () => {
    ...
    const hit: BoundaryHit = { kind: 'finite', he, point: { x: 1, y: 0 }, u: 0.2 };
    const hit2: BoundaryHit = { kind: 'finite', he, point: { x: 4, y: 0 }, u: 0.8 };
    assert.throws(() => splitFaceAlongChord(atlas, quad, hit, hit2), /same half-edge/);
  });
  ...
  it('throws when an entry hit is not on the given face', () => {
    ...
    assert.throws(() => splitFaceAlongChord(atlas, ne, entryHit, exitHit), /not on face/);
  });
...
describe('mergeFaces', () => {
  ...
  it('refuses to merge when the chord stitch is not a pure translation', () => {
    ...
    split.faceChordSide.stitch!.transformAtoB = refl;
    split.faceChordSide.stitch!.transformBtoA = M.invert(refl);
    assert.throws(() => mergeFaces(atlas, split.faceChordSide), /pure translation/);
  });
  ...
  it('refuses to merge when fresh face has shapes', () => {
    ...
    assert.throws(() => mergeFaces(atlas, split.faceChordSide), /has 1 shape\(s\) assigned/);
  });
  ...
  it('refuses to merge when fresh face is the source/target of a Link', () => {
    ...
    assert.throws(() => mergeFaces(atlas, split.faceChordSide), /source or target of a Link/);
  });
...
describe('splitAlongLine (propagating)', () => {
  ...
  it('returns pairs in line-traversal order from -direction infinity to +direction infinity', () => {
    ...
    assert.ok(hasIdeal(last, -1, 0) && hasIdeal(last, 0, -1), 'last crossed face should be SW wedge');
  });
  ...
  it('chord HEs across the chain form continuous twin pairs at shared chord-vertices', () => {
    ...
    for (let i = 0; i < result.pairs.length - 1; i++) {
      ...
      const hasLeftConnection = a.leftFace.sides.some((h) => h.twin && h.twin.face === b.leftFace);
      const hasRightConnection = a.rightFace.sides.some((h) => h.twin && h.twin.face === b.rightFace);
    ...
    for (const { name, cx, cy } of cases) {
      it(`carves a rectangle inside ${name} without throwing`, () => {
        ...
        for (const [x, y] of [
          ...
          assert.ok(atlas.locate({ x, y }), `lost coverage near rect corner (${x}, ${y}) in ${name}`);
        }
      });
    }
  });
...
describe('splitAlongLine (face-bounded)', () => {
  ...
  it('only subdivides boundary HEs of the host face (and their twins, never the twin face structure)', () => {
    ...
    assert.equal(nwHesAfter, nwHesBefore + 1, 'NW should gain exactly one boundary subdivision, not be split');
  });
  ...
  it('throws when seam is not strictly interior to host', () => {
    ...
    assert.throws(
      () => splitAlongLine(atlas, ne, { x: 0, y: 0 }, { x: 1, y: 0 }, { propagate: false }),
  ...
  it('throws on zero-length direction', () => {
    ...
    const ne = atlas.locate({ x: 1, y: 0.5 })!;
    assert.throws(
      () => splitAlongLine(atlas, ne, { x: 1, y: 0.5 }, { x: 0, y: 0 }, { propagate: false }),
  ...
  it('throws when host is not in the atlas', () => {
    ...
    assert.throws(
      () => splitAlongLine(atlas, stranger, { x: 1, y: 0.5 }, { x: 1, y: 0 }, { propagate: false }),
      /host not in atlas/,
    );
  });
  ...
  it('cuts an all-ideal seed face into two half-planes (line-through-the-empty-canvas)', () => {
    ...
    const top = result.pairs[0].rightFace.contains({ x: 0, y: 1 })
      ? result.pairs[0].rightFace
      : result.pairs[0].leftFace;
  ...
  it('produces a digon (slab) sub-face when cutting a half-plane parallel to its chord', () => {
    ...
    assert.ok(digon!.sides.every((h) => h.kind === 'chord'));
...
describe('insertStrip', () => {
  ...
  it('rejects empty chain (N=0)', () => {
    const atlas = createInitialAtlas();
    assert.throws(() => insertStrip(atlas, { pairs: [] }, 0.5), /at least one face crossing/);
  });
  ...
  it('strip frame has a finite vertex at (0, 0)', () => {
    ...
    const { atlas, splitResult } = setupChain();
    const result = insertStrip(atlas, splitResult, 0.5);
    const origin = result.stripFace.junctions().find((j) => j.kind === 'finite' && j.x === 0 && j.y === 0);
  ...
  it('strip face contains an interior point (height/2 from each chord)', () => {
    ...
    assert.ok(result.stripFace.contains(mid), `strip should contain interior midpoint ${JSON.stringify(mid)}`);
  });
  ...
  it('strip top edges are translated copies of bottom edges (height apart)', () => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (botOrigin.kind === 'finite' && topTarget.kind === 'finite') {
        assert.ok(Math.abs(topTarget.x - (botOrigin.x + 0.5 * perp.x)) < 1e-9, `top.target.x mismatch at step ${i}`);
        assert.ok(Math.abs(topTarget.y - (botOrigin.y + 0.5 * perp.y)) < 1e-9, `top.target.y mismatch at step ${i}`);
  ...
  it('twin transforms left → strip → right encode the perp gap of size height', () => {
    ...
    assert.ok(Math.abs(o.x - expected.x) < 1e-9, `composed offset x: got ${o.x}, expected ${expected.x}`);
    assert.ok(Math.abs(o.y - expected.y) < 1e-9, `composed offset y: got ${o.y}, expected ${expected.y}`);
    void N;
  });
  ...
  it('produces a digon strip when inserted into a single ideal-ideal chord (all-ideal seed)', () => {
    ...
    assert.ok(result.stripFace.sides.every((h) => h.kind === 'chord'));
  ...
  it('two consecutive diagonal cuts on the all-ideal seed keep the atlas valid', () => {
    ...
    const splitResult2 = splitAlongLine(atlas, host2, { x: 0, y: 0.25 }, { x: 1 / Math.sqrt(2), y: -1 / Math.sqrt(2) });
...
describe('resizeStrip', () => {
  ...
  it('no-op when oldHeight equals newHeight', () => {
    ...
    const topBefore = stripResult.topSides.map((h) => ({ x: h.a.x, y: h.a.y }));
  ...
  it('shifts finite topHE origins by Δ·perp', () => {
    ...
    for (let i = 0; i < stripResult.topSides.length; i++) {
      ...
      if (topBefore[i].kind === 'finite') {
        assert.ok(
          Math.abs(t.a.x - (topBefore[i].x + delta * perp.x)) < 1e-9,
          `topSides[${i}].a.x: got ${t.a.x}, expected ${topBefore[i].x + delta * perp.x}`,
        );
        assert.ok(
          Math.abs(t.a.y - (topBefore[i].y + delta * perp.y)) < 1e-9,
          `topSides[${i}].a.y: got ${t.a.y}, expected ${topBefore[i].y + delta * perp.y}`,
        );
      } else {
        assert.equal(t.a.kind, 'ideal');
        assert.ok(Math.abs(t.a.x - topBefore[i].x) < 1e-12);
        assert.ok(Math.abs(t.a.y - topBefore[i].y) < 1e-12);
      }
    }
  });
