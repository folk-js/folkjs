...
type ChessPosition = `${ChessColumn}${ChessRow}`;
...
type ChessBoardConfiguration = [ChessPosition, ChessPieceType][];

const defaultBoard: ChessBoardConfiguration = [
  ['a1', 'white-rook'],
  ['b1', 'white-knight'],
  ['c1', 'white-bishop'],
  ['d1', 'white-queen'],
...
class ChessBoard extends ReactiveElement {
  ...
  configureBoard(configuration: ChessBoardConfiguration = defaultBoard) {
    this.textContent = '';
    const pieces = configuration.map(([position, type]) => this.createPiece(position, type));
    this.append(...pieces);
  }

  createPiece(position: ChessPosition, type: ChessPieceType) {
